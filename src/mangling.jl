# name mangling copy from GPUCompile.jl project
...
function mangle_param(t, substitutions = Any[], top = false)
    ...
    function find_substitution(x)
        ...
        elseif sub == 1
            ...
            seq_id = uppercase(string(sub-2; base=36))+1
            "S$(seq_id)_"
        end
        return nothing
    end
...
@inline function module_path(m::Core.Module)
	v = Array{Symbol, 1}()
	pM = parentmodule(m)
	while(pM != m)
		push!(v, nameof(m))
    ...
    if length(v) > 0 && m !== Main
	    push!(v, nameof(m)) # skip default Main Module
    end
	return v
end

@inline _add_num(s::String) = string(length(s)) * s
@inline _add_num(s) = _add_num(string(s))

@inline function mangle_NS(name::Symbol, mod::Module)
    name = _add_num(name)

    my_modules = module_path(mod)
    my_modules = map(_add_num, reverse(my_modules))

    ret = "_ZN" * join(my_modules) * "$(name)E"
    return replace(ret, '#'=>'_', '@'=>'_')
end

@inline function mangle_NS(mt::Core.Method)
	name = _add_num(mt.sig.parameters[1].instance)
	#n_name = length(name)
	
	#_filename = replace(string(mt.file), r"[()\[\]\W\{\}]" => "_")
	#filename = _add_num(_filename)

	my_modules = map(_add_num, module_path(mt.module))
	#push!(my_modules, filename)
	my_modules = reverse(my_modules)
	new_name = join(my_modules)
	
	old_mangle = mangle_sig(mt.sig)
	
	mangle = "_ZN" * new_name * old_mangle[4:end]
	return mangle
end
